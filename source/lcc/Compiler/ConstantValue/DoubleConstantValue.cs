...
{
  internal class DoubleConstantValue : FloatingConstantValue
  {
    public double Value { get; private set; }

    public DoubleConstantValue(double value) : base(new LCPrimitiveType(LCPrimitiveType.PrimitiveTypes.LCTypeDouble))
    {
      Value = value;
    }
    ...
    {
      if (rightValue is DoubleConstantValue rightDoubleConstant)
        return new DoubleConstantValue(Value + rightDoubleConstant.Value);
    ...
    {
      if (rightValue is DoubleConstantValue rightDoubleConstant)
        return new BooleanConstantValue(Value < rightDoubleConstant.Value);
    ...
    {
      return new DoubleConstantValue(Value + 1);
    }
      ...
      if (toType is LCPrimitiveType primitiveType)
      {
        switch (primitiveType.Type)
        {
          ...
          case LCPrimitiveType.PrimitiveTypes.LCTypeFloat:
            return new FloatConstantValue((float)Value);
